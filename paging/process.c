...
void init_process_table(PCB* process_table, int seed_value) {
  for (int i = 0; i < MAX_PROCESSES; i++) {
      ...
      process_table[i].time_spent_memory_access[0] = seed_value;
      process_table[i].time_spent_memory_access[1] = seed_value;
      process_table[i].state = -1;
      process_table[i].total_times_run = seed_value;
      process_table[i].total_times_blocked = seed_value;
      process_table[i].total_memory_requests = seed_value;
  }
}
...
void init_process(PCB* process, int seed) {
  ...
  process->time_spent_memory_access[0] = seed;
  process->time_spent_memory_access[1] = seed;
  process->state = -1;
  process->total_times_run = seed;
  process->total_times_blocked = seed;
  process->total_memory_requests = seed;
}
...
int insert_process(PCB* process_table, pid_t pid, int start_seconds, int start_nano) {
  ...
  for (int i = 0; i < MAX_PROCESSES; i++) {
    if (!process_table[i].occupied) {
      ...
      process_table[i].time_spent_memory_access[0] = 0;
      process_table[i].time_spent_memory_access[1] = 0;
      process_table[i].state = READY;
      process_table[i].total_times_run = 0;
      process_table[i].total_times_blocked = 0;
      process_table[i].total_memory_requests = 0;
...
void display_process_table(PCB* process_table) {
  ...
  printf("Entry    Occupied PID     Run     Blocked  State   Time Ready     Time Blocked\n");
  for (int i = 0; i < MAX_PROCESSES; i++) {
    printf("%-8d %-8d %-8d %-8d %-8d %-8d %-llu, %-8llu %-llu, %-8llu\n", i, process_table[i].occupied, process_table[i].pid, process_table[i].total_times_run, process_table[i].total_times_blocked, process_table[i].state, process_table[i].time_spent_ready[0], process_table[i].time_spent_ready[1], process_table[i].time_spent_blocked[0], process_table[i].time_spent_blocked[1]);
  }
}
...
// add_to_memory_access_time
// updates the time spent making a memory request
//
// Input: PCB* process_table, int time, int pid, int just_use_blocked
// The PID for the process, the time spent in the system to add, the pid of the process we want to use, a boolean if we just want to add memory access time to blocked processes
//
// Output: void
void add_to_memory_access_time(PCB* process_table, int time, int pid, int just_use_blocked) {
  
  if (!just_use_blocked) {
    for (int i = 0; i < MAX_PROCESSES; i++) {
      if (process_table[i].pid == pid) {
        process_table[i].time_spent_memory_access[1] += time;
        
        update_time_if_billion(&process_table[i].time_spent_memory_access[0], &process_table[i].time_spent_memory_access[1]);
      }
    }
  } else {
    for (int i = 0; i < MAX_PROCESSES; i++) {
...
// set_blocked_memory_access_time
// Set the blocked memory's access time
//
// Input: PCB* process_table, int pid, unsigned long long int s, unsigned long long int ns
// process_table : the process table to update information from
// pid           : the process we want to update
// s             : the seconds we want to update to
// ns            : the nanoseconds we want to update to
//
// Output: void
void udpate_blocked_memory_access_time(PCB* process_table, int pid, unsigned long long int s, unsigned long long int ns) {
  for (int i = 0; i < MAX_PROCESSES; i++) {
    if (process_table[i].pid == pid) {
      process_table[i].time_spent_memory_access[0] = s;
      process_table[i].time_spent_memory_access[0] = ns;
      update_time_if_billion(&process_table[i].time_spent_memory_access[0], &process_table[i].time_spent_memory_access[1]);
    }
  }
}

// increment_total_memory_requests
// Increments the number of times we made a memory request
//
// Input: PCB* process_table, int pid
// The PID for the process and the pid of the process we want to use
//
// Output: void
void increment_total_memory_requests(PCB* process_table, int pid) {
  for (int i = 0; i < MAX_PROCESSES; i++) {
    if (process_table[i].pid == pid) {
      process_table[i].total_memory_requests++;
    }
  }
...
PCB get_process_copy(PCB* process_table, int pid) {
  ...
  for (int i = 0; i < MAX_PROCESSES; i++) {
    if (process_table[i].pid == pid) {
      ...
      temp_pcb.time_spent_memory_access[0] = process_table[i].time_spent_memory_access[0];
      temp_pcb.time_spent_memory_access[1] = process_table[i].time_spent_memory_access[1];
      temp_pcb.total_times_run = process_table[i].total_times_run;
      temp_pcb.total_times_blocked = process_table[i].total_times_blocked;
      temp_pcb.total_memory_requests = process_table[i].total_memory_requests;
      return temp_pcb;
    }
  }
