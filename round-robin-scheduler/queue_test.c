...
int main() {
  ...
  queue_insert(&queue_head, 3, 4000, 4000);
  display_queue(&queue_head);

  int keep_going = 1;
  while ((keep_going = queue_remove_blocked(&queue_head, 1000, 1000)) != -1) {
    printf("Removing queue node, got a value of %d\n", keep_going);
    display_queue(&queue_head);
  }
  
  printf("Final queue, got a value of %d\n", keep_going);
  display_queue(&queue_head);
